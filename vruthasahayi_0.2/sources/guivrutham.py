...
class myPanel(wx.Panel):
	def __init__(self, parent, id):
		...
		# Buttons
		self.goButton = wx.Button(self, 10, data.findLabel, wx.Point(379, 470), wx.Size(200,35))
	...
	def goClick(self,event):
		...
		if self.input.GetValue() == '':
			...
			self.increaseCellFontSize(0, 0)
			return
		if [x for x in self.input.GetValue() if x > '\u0d00' and x < '\u0d65'] == '':
			self.output.SetDefaultColSize(790, 1)
			self.giveCell()
